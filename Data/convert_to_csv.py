...
# Inputs
Model = "ARAP"              # "ARAP_NoGlobal" "ARAP", "Elastic, HyperElastic"
Triangulation = "InRays"    # InRays" or "TwoPoints"
Depth = 20
Shape = "planar"
gaussianMov = 2.5
rigidMov = 0
...
with open('./Data/Experiment.txt', 'r') as txt_file:
    lines = txt_file.readlines()

# Step 2: Initialize variables
firstMeasure = False
if Depth == 20 and Shape == "planar" and gaussianMov == 2.5 and rigidMov == 0:
    firstMeasure = True
...
if initial_av_error is not None and final_av_error is not None:
    try:
        improv_percentage = ((float(final_av_error.replace(',', '.')) - float(initial_av_error.replace(',', '.'))) / 
                             float(final_av_error.replace(',', '.'))) * 100
    except ValueError:
...
with open('./Data/'+ Model + "_" + Triangulation + '.csv', file_mode, newline='') as csv_file:
...
csv_file = './Data/'+ Model + "_" + Triangulation + '.csv'
df = pd.read_csv(csv_file)

# Save the Data as an Excel file
excel_file = './Data/'+ Model + "_" + Triangulation + '.xlsx'
